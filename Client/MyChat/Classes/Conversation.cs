...
namespace MyChat
{
    class Conversation
    {
        ...
        public string Name_cvs { get; set; }
            ...
            get
            {
                ...
                foreach (var mes in Messages)
                {
                    if (mes.Sender.Equals(Sender.Me))
                        conver += "<div class='message mine' title='" + mes.Sender.NickName + " " + mes.Time.ToString("HH:mm:ss dd/MM/yy") + "'style='border-radius: 10px;box-shadow: 0px 2px 2px 0px;'>" + mes.Content + "</div>\n";
                    else
                    {
                        conver += "<div class='message remote' title='" + mes.Sender.NickName + " " + " " + mes.Time.ToString("HH:mm:ss dd/MM/yy") + "'";
                        if (mes.Sender.Color.Name != "0")
                            conver += " style='background:" + GetHTMLColor(mes.Sender.Color) + "'";
                        conver += ">"+"<b>" + mes.Sender.NickName + "</b>: " + mes.Content + "</div>";
                    }
                }
